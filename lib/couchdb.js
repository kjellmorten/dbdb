  ...
  connect () {
    ...
    if (this._db === null) {
      this._db = new Promise((resolve, reject) => {
        ...
        const conn = nano({ url: this.config.url, requestDefaults: requestDefaults }, () => {})
        const db = conn.use(this.config.db)
          ...
          conn.auth(this.config.key, this.config.password, (err, body, headers) => {
            ...
            if (headers && headers['set-cookie']) {
              ...
              if (Array.isArray(cookie)) {
                const index = cookie.findIndex((str) => str.startsWith('AuthSession='))
                cookie = cookie[index]
              }
              if (cookie) {
